...
const EditPost = () => {
  ...
  useEffect(() => {
    if (post && user && (post.author?._id || post.author) !== user._id) {
      ...
      payload,
      {
        onSuccess: async () => {
          if (coverFile) {
            ...
            try {
              await api.post(`/posts/${id}/cover`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
              });
            } catch {
              toast.error('Post updated but cover upload failed');
            } finally {
    ...
    <AppLayout sidebar={false}>
      <div className="max-w-4xl mx-auto animate-fade-in">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-[var(--color-text-primary)]">Edit Post</h1>
          <button
            type="button"
